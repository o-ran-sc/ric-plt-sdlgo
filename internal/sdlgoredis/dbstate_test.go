...
func (ds *dbStateMock) setMasterFields(role, ip, port, rCnt, rcCnr, flags string) {
	...
	ds.state.MasterDbState.Fields.ReplicasCfgCnt = rcCnr
...
func TestIsOnlineWhenSingleMasterSuccessfully(t *testing.T) {
	st := setupDbState()
	st.setMasterFields("master", "1.2.3.4", "60000", "0", "0", "master")
...
func TestIsOnlineWhenSingleMasterFailureIfNotMasterRole(t *testing.T) {
	...
	st := setupDbState()
	st.setMasterFields("not-master", "1.2.3.4", "60000", "0", "0", "master")
	err := st.state.IsOnline()
	assert.Equal(t, expErr, err)
}

func TestIsOnlineWhenMasterFailureIfNotAllReplicasOnline(t *testing.T) {
	expErr := errors.New("Master has 1 replicas, expected 2 replicas")
	st := setupDbState()
	st.setMasterFields("master", "1.2.3.4", "60000", "1", "2", "master")
	err := st.state.IsOnline()
	assert.Equal(t, expErr, err)
}
...
func TestIsOnlineWhenSingleMasterFailureIfErrorFlags(t *testing.T) {
	...
	st := setupDbState()
	st.setMasterFields("master", "1.2.3.4", "60000", "0", "0", "any-error,master")
	err := st.state.IsOnline()
	assert.Equal(t, expErr, err)
}
...
func TestGetAddressMasterFailureNoIpPort(t *testing.T) {
	st := setupDbState()
	st.setMasterFields("master", "", "", "0", "0", "master")
	addr := st.state.MasterDbState.GetAddress()
	assert.Equal(t, "", addr)
}
...
func TestIsOnlineWhenMasterAndTwoReplicasSuccessfully(t *testing.T) {
	st := setupDbState()
	st.setMasterFields("master", "1.2.3.4", "60000", "2", "2", "master")
