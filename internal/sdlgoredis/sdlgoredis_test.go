...
/*
func subscribeNotifications(client sdlgoredis.RedisClient, channels ...string) sdlgoredis.Subscriber {

	return new(pubSubMock)
}
*/
...
func TestMSetMPubSuccessfully(t *testing.T) {
	...
	assert.Nil(t, db.MSetMPub([]string{"chan1", "event1", "chan2", "event2"}, "key1", "val1", "key2", "val2"))
	r.AssertExpectations(t)
}
...
func TestMsetMPubFailure(t *testing.T) {
	...
	assert.NotNil(t, db.MSetMPub([]string{"chan1", "event1", "chan2", "event2"}, "key1", "val1", "key2", "val2"))
	r.AssertExpectations(t)
}
...
func TestMGetFailure(t *testing.T) {
	...
	r.On("MGet", expectedKeys).Return(redis.NewSliceResult(expectedResult, errors.New("Some error")))
	result, err := db.MGet([]string{"key1", "key2", "key3"})
	assert.Equal(t, result, expectedResult)
	assert.NotNil(t, err)
	r.AssertExpectations(t)
...
func TestDelMPubSuccessfully(t *testing.T) {
	_, r, db := setup(true)
	expectedMessage := []interface{}{"DELMPUB", 2, 2, "key1", "key2", "chan1", "event1", "chan2", "event2"}
	r.On("Do", expectedMessage).Return(redis.NewCmdResult("", nil))
	assert.Nil(t, db.DelMPub([]string{"chan1", "event1", "chan2", "event2"}, []string{"key1", "key2"}))	
	r.AssertExpectations(t)
}
...
func TestDelMPubFailure(t *testing.T) {
	...
	expectedMessage := []interface{}{"DELMPUB", 2, 2, "key1", "key2", "chan1", "event1", "chan2", "event2"}
	r.On("Do", expectedMessage).Return(redis.NewCmdResult("", errors.New("Some error")))
	assert.NotNil(t, db.DelMPub([]string{"chan1", "event1", "chan2", "event2"}, []string{"key1", "key2"}))	
	r.AssertExpectations(t)
}
...
func TestKeysFailure(t *testing.T) {
	...
	r.On("Keys", expectedPattern).Return(redis.NewStringSliceResult(expectedResult, errors.New("Some error")))
...
func TestSetNXFailure(t *testing.T) {
	...
	expectedKey := "key"
	expectedData := "data"
	r.On("SetNX", expectedKey, expectedData, time.Duration(0)).Return(redis.NewBoolResult(false, errors.New("Some error")))
	result, err := db.SetNX("key", "data")
	assert.False(t, result)
	assert.NotNil(t, err)
	r.AssertExpectations(t)
...
func TestSMembersFailure(t *testing.T) {
	...
	expectedKey := "key"
	expectedResult := []string{"member1", "member2"}
	r.On("SMembers", expectedKey).Return(redis.NewStringSliceResult(expectedResult, errors.New("Some error")))
	result, err := db.SMembers("key")
	assert.Equal(t, result, expectedResult)
	assert.NotNil(t, err)
	r.AssertExpectations(t)
...
func TestSIsMemberFailure(t *testing.T) {
	...
	expectedKey := "key"
	expectedData := "data"
	r.On("SIsMember", expectedKey, expectedData).Return(redis.NewBoolResult(false, errors.New("Some error")))
	result, err := db.SIsMember("key", "data")
	assert.False(t, result)
	assert.NotNil(t, err)
	r.AssertExpectations(t)
