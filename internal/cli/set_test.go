...
func TestCliSetCanShowHelp(t *testing.T) {
	...
	expArgCntLtErr := fmt.Errorf("requires at least 3 arg(s), only received 2")
	expArgCntGtErr := fmt.Errorf("requires only 3 arg(s), received 4")
	tests := []struct {
		args      []string
		expErr    error
		expOutput string
...
func TestCliSetCommandFailure(t *testing.T) {
	expErr := fmt.Errorf("SDL CLI error: some-error")
	setupSetCliMock("some-ns", "some-key", "some-value", fmt.Errorf("some-error"))

	stdout, err := runSetCli()

	assert.Equal(t, expErr, err)
	assert.Contains(t, stdout, expErr.Error())
	setMocks.sdlIface.AssertExpectations(t)
}
