...
func init() {
	getCmd.AddCommand(newKeysCmd(newSdl))
}
...
func newKeysCmd(sdl SdlCreateCb) *cobra.Command {
	return &cobra.Command {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cli := newCli(sdl, args[0], "*")
			if len(args) > 1 {
				cli.pattern = args[1]
			}
			keys, err := runListKeys(cli)
			if err != nil {
...
func runListKeys(c Cli) ([]string, error) {
	keys, err := c.ListKeys()
	if err != nil {
		sort.Strings(keys)
	}
